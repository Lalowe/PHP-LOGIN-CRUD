...
if ($_GET['id']) {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        $resultSup = mysqli_query($connect, "SELECT * FROM agency");
        $supList = "";
        if (mysqli_num_rows($resultSup) > 0) {
            while ($row = $resultSup->fetch_array(MYSQLI_ASSOC)) {
                if ($row['agencyId'] == $agency) {
                    $supList .= "<option selected value='{$row['agencyId']}'>{$row['agency_name']}</option>";
                } else {
                    $supList .= "<option value='{$row['agencyId']}'>{$row['agency_name']}</option>";
                }
            }
        } else {
            $supList = "<li>There are no agency registered</li>";
        }
    } else {
