...
if (isset($_POST['btn-signup'])) {
    $fname = cleanup($_POST["fname"]); // "fname" is coming of the input form not from the database but recommend to use same names !!!!
    $lname = cleanup($_POST["lname"]); // berie to akoparameter z inputu 
    $email = cleanup($_POST["email"]);
    $date_of_birth = cleanup($_POST["date_of_birth"]);
    ...
    if (empty($fname) || empty($lname)) { // if there is any input empty , our erros becomne true !! and we are not able to inser
        $error = true;
        $fnameError = "Please enter your full name and surname";// cize ak neni prazdne idem dalej
    } else if (strlen($fname) < 3 || strlen($lname) < 3) { // string length uz su pouzil it takes parameter
        $error = true;
        $fnameError = "Name and surname must have at least 3 characters.";// ak neni prazdne a je dlhsie ako 3 
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        // checks whether the email exists or not if exist then we write it is already used
        $query = "SELECT email FROM users WHERE email='$email'";
        $result = mysqli_query($connect, $query); // we always need to run our query
    ...
    if (!$error) { // cize ak nie su errory vloz do databazy  cize asi treba vytvorit table users
